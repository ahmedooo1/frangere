...
RSS feeds, and uses Claude to filter, translate (Arabic), summarize, and
...
- **`@anthropic-ai/sdk`** (Claude) for filter → translate → summarize → categorize, with a deterministic mock fallback when no API key is set
...
npm install
cp .env.example .env
# edit .env: DATABASE_URL, ANTHROPIC_API_KEY (optional — mock fallback works without it)
```
...
Without `ANTHROPIC_API_KEY` set, the pipeline still runs end-to-end and
...
The pipeline (poll feeds → Claude → save `PUBLISHED` articles) runs three ways:
...
server/
  ...
  utils/
    ai.ts              # Claude prompt + mock fallback
...
- Set `CRON_SECRET` in production so `POST /api/feed` can't be spammed by
  the public (it consumes Claude API quota).
